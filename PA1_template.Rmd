...
author: "submitted by: Googlehupf"
date: "2015-10-18"
output: 
  html_document:
    keep_md: true
...
```
The total number of steps taken per day has a mean of ```r steps.mean```, and a median of ```r steps.median```.
...
```
The interval with the maximum number of steps is ```r max.steps```, corresponding to a value of ```r time.max.steps```, i.e. 8:35h in the morning.
